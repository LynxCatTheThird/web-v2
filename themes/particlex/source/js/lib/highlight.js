(function() {
    ...
    window.renderers.push(() => {
        ...
        for (let i of codes) {
            ...
            let language = [...i.classList, ...(i.firstChild && i.firstChild.classList ? i.firstChild.classList : [])][0] || "plaintext";
            let highlighted;
            try {
            ...
            // Optimize folding: auto-fold if code has more lines than the threshold
            if (code.split("\n").length > theme_config.codeFoldThreshold) {
            ...
            copyBtn.addEventListener("click", async () => {
                ...
                } catch { /* clipboard API unavailable in non-secure context */ }
                await sleep(1000);
